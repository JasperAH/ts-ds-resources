...
public class OneMeasurementHistogram extends OneMeasurement {
  ...
  public void exportMeasurements(MeasurementsExporter exporter) throws IOException {
    ...
    exporter.write(getName(), "OneMeasurementHistogram", 0);
    ...
    lowWhisker = val25 - 1.5*(val75-val25);
    highWhisker = val75 + 1.5*(val75-val25);

    if(lowWhisker < 0){
      lowWhisker = 0;
    }
    if(highWhisker >= buckets){
      highWhisker = buckets-1;
    }

    for (int i = lowWhisker; i < buckets; i++){
      lowWhisker = i;
      if(histogram[i]>0){
        break;
      }
    ...
    ArrayList<Integer> lowOutliers = new ArrayList<Integer>();
    ArrayList<Integer> highOutliers = new ArrayList<Integer>();

    for (int i = 0; i < buckets; i++) {
      if (i < lowWhisker && histogram[i] > 0){
        lowOutliers.add(i*1000);
      } else if (i > highWhisker && histogram[i] > 0){
        highOutliers.add(i*1000);
      }
    }
